...
export default function BeerPong({ partyId, sessionId, onSave }: { partyId: string; sessionId?: string | null; onSave: (score: number) => void }) {
  const { t } = useLocale();
  ...
  return <div className="party-game-board game-board-enter">
    ...
    <div className="game-primary-actions">
      <button className="demo-action demo-action--lime" onClick={() => onSave(20 - state.scores[0] - state.scores[1])} type="button">{t("beerSave")}</button>
    </div>
