...
type GameState = {
  ...
  prompt: string; submissions: Submission[];
  votes: Record<string, string>; scores: Record<string, number>;
};

function QuiplashStage({ sessionId, partyId, onSave }: { sessionId?: string | null; partyId: string; onSave: (score: number) => void }) {
  ...
  const { state, setState, playerActions, clearActions, complete } = useStageGame<GameState>(
    sessionId ?? null,
    () => ({ round: 0, phase: "answer", prompt: prompts[0], submissions: [], votes: {}, scores: {} }),
  );
  ...
  const reveal = useCallback(() => {
    ...
    for (const [uid, count] of Object.entries(tally)) {
      newScores[uid] = (newScores[uid] || 0) + count;
    }
  ...
  const next = useCallback(() => {
    ...
    if (nextRound >= Math.min(prompts.length, 6)) {
      complete();
    ...
    setState({
      round: nextRound, phase: "answer", prompt: prompts[nextRound],
      submissions: [], votes: {}, scores: state.scores,
    });
  }, [state.round, state.scores, prompts, setState, complete, onSave]);
      ...
      {state.phase === "reveal" && (
        <div>
          ...
          <button className="demo-action demo-action--lime" onClick={next} type="button" style={{ marginTop: 8 }}>
            {state.round >= Math.min(prompts.length, 6) - 1 ? t("finish") : t("next")}
          </button>
        </div>
      )}
...
function QuiplashController({ sessionId }: { sessionId: string }) {
  const { locale } = useLocale();
  const t = (key: string) => locale === "ru" ? (RU[key] ?? key) : (EN[key] ?? key);
  const { state, sendAction } = useControllerGame<GameState>(sessionId, {
    round: 0, phase: "answer", prompt: "", submissions: [], votes: {}, scores: {},
  });
...
const EN: Record<string, string> = {
  round: "Round", waitingAnswers: "Waiting for answers…", votePrompt: "Vote for the best!",
  reveal: "Reveal", votes: "votes", finish: "Finish", next: "Next",
  answerPlaceholder: "Type your answer…", submit: "Submit", submitted: "Sent!",
  vote: "Vote", revealed: "Results shown on stage", voted: "Voted!",
};

const RU: Record<string, string> = {
  round: "Раунд", waitingAnswers: "Ждём ответы…", votePrompt: "Голосуй за лучший!",
  reveal: "Показать", votes: "голосов", finish: "Завершить", next: "Далее",
  answerPlaceholder: "Введи ответ…", submit: "Отправить", submitted: "Отправлено!",
  vote: "Голосовать", revealed: "Результаты на сцене", voted: "Готово!",
};
...
export default function Quiplash({ partyId, sessionId, onSave, role }: { partyId: string; sessionId?: string | null; onSave: (score: number) => void; role?: "stage" | "controller" }) {
  if (role === "controller" && sessionId) return <QuiplashController sessionId={sessionId} />;
  return <QuiplashStage sessionId={sessionId} partyId={partyId} onSave={onSave} />;
}
