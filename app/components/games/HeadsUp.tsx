...
const WORDS_EN = [
  "Elephant", "Pizza", "Karaoke", "Unicorn", "Ninja", "Sushi", "Volcano", "Penguin",
  "Batman", "Yoga", "Chandelier", "Tornado", "Giraffe", "Disco", "Lobster", "Robot",
  "Cactus", "Dinosaur", "Mermaid", "Yeti", "Zombie", "T-Rex", "Vampire", "Alien",
  "Ghost", "Pirate", "Snowman", "Zombie", "Caveman", "Gladiator",
];

const WORDS_RU = [
  "Слон", "Пицца", "Караоке", "Единорог", "Ниндзя", "Суши", "Вулкан", "Пингвин",
  "Бэтмен", "Йога", "Люстра", "Торнадо", "Жираф", "Диско", "Лобстер", "Робот",
  "Кактус", "Динозавр", "Русалка", "Йети", "Зомби", "Ти-Рекс", "Вампир", "Инопланетянин",
  "Призрак", "Пират", "Снеговик", "Зомби", "Пещерный человек", "Гладиатор",
];
...
type GameState = {
  round: number; phase: "play" | "result";
  words: string[]; currentIndex: number; score: number;
  activePlayer: string; timer: number; direction: "correct" | "skip" | null;
};

function HeadsUpStage({ sessionId, partyId, onSave }: { sessionId?: string | null; partyId: string; onSave: (score: number) => void }) {
  ...
  const { state, setState, playerActions, clearActions, complete } = useStageGame<GameState>(
    sessionId ?? null,
    () => ({
      round: 0, phase: "play" as const, words, currentIndex: 0, score: 0,
      activePlayer: "", timer: 60, direction: null,
    }),
  );
  ...
  useEffect(() => {
    ...
    for (const a of playerActions) {
      if (a.actionType === "correct" && state.phase === "play") {
        setState((prev) => ({
          ...prev, score: prev.score + 1, currentIndex: prev.currentIndex + 1, direction: "correct",
        }));
      }
      if (a.actionType === "skip" && state.phase === "play") {
        setState((prev) => ({
          ...prev, currentIndex: prev.currentIndex + 1, direction: "skip",
        }));
      }
  ...
  const reset = useCallback(() => {
    setState({
      round: state.round + 1, phase: "play", words: shuffle(locale === "ru" ? WORDS_RU : WORDS_EN),
      currentIndex: 0, score: 0, activePlayer: "", timer: 60, direction: null,
    });
      ...
      {state.phase === "play" && (
        <div>
          ...
          <p style={{ color: "var(--gray)", marginTop: 8 }}>
            {t("score")}: {state.score} | {t("word")} {state.currentIndex + 1}/{totalWords}
          </p>
          {state.direction && (
            <p style={{ color: state.direction === "correct" ? "var(--lime)" : "var(--red)", fontWeight: 700, marginTop: 4 }}>
              {state.direction === "correct" ? `✓ ${t("correct")}` : `⏭ ${t("skipped")}`}
            </p>
          )}
          {!state.activePlayer && (
...
function HeadsUpController({ sessionId }: { sessionId: string }) {
  const { locale } = useLocale();
  const t = (key: string) => locale === "ru" ? (RU[key] ?? key) : (EN[key] ?? key);
  const { state, sendAction } = useControllerGame<GameState>(sessionId, {
    round: 0, phase: "play", words: [], currentIndex: 0, score: 0,
    activePlayer: "", timer: 60, direction: null,
  });
...
export default function HeadsUp({ partyId, sessionId, onSave, role }: { partyId: string; sessionId?: string | null; onSave: (score: number) => void; role?: "stage" | "controller" }) {
  if (role === "controller" && sessionId) return <HeadsUpController sessionId={sessionId} />;
  return <HeadsUpStage sessionId={sessionId} partyId={partyId} onSave={onSave} />;
}
