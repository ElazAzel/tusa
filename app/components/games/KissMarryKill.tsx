...
export default function KissMarryKill({ sessionId, onSave, role }: { partyId: string; sessionId?: string | null; onSave: (score: number) => void; role?: "stage" | "controller" }) {
  ...
  const copy = locale === "ru" ? { title: "Поцелуй / Брак / Убить", round: "Раунд", hint: "Назначь каждому свой вариант, каждый используется один раз", actions: ["Поцеловать", "Пожениться", "Исключить"], submit: "Подтвердить выбор", accepted: "Выбор принят", reveal: "Показать итоги", next: "Следующий раунд", finish: "Завершить", votes: "ответов" } : { title: "Kiss / Marry / Kill", round: "Round", hint: "Assign one option to each person — use every option once", actions: ["Kiss", "Marry", "Drop"], submit: "Lock choices", accepted: "Choices locked", reveal: "Reveal results", next: "Next round", finish: "Finish", votes: "responses" };
