...
export default function UnoTracker({ sessionId, onSave, role }: { partyId: string; sessionId?: string | null; onSave: (score: number) => void; role?: "stage" | "controller" }) {
  ...
  return <div className="party-game-board game-board-enter uno-multiplayer">
    <span className="game-step">UNO · {state.phase === "lobby" ? (ru ? "Лобби" : "Lobby") : `${state.currentIndex + 1}/${state.players.length}`}</span>
    {state.phase === "lobby" && <><h3>{ru ? "Собираем игроков" : "Players are joining"}</h3><div className="uno-player-chips">{state.players.map((id, index) => <span key={id}>{index + 1}. {id === me ? (ru ? "Ты" : "You") : id.slice(-6)}</span>)}</div>{isHost && <button className="demo-action demo-action--lime" disabled={state.players.length < 2} onClick={start} type="button">{ru ? "Раздать карты" : "Deal cards"}</button>}</>}
    {state.phase !== "lobby" && <><div className="uno-table"><button className="uno-card uno-card--back" disabled={!myTurn} onClick={() => sendAction("draw")} type="button">+{state.drawPile.length}</button>{top && <div className={`uno-card uno-card--${top.color ?? "wild"}`}><strong>{label(top)}</strong><small>{state.activeColor}</small></div>}</div><p className={myTurn ? "controller-answered" : ""}>{myTurn ? (ru ? "Твой ход" : "Your turn") : (ru ? "Ход другого игрока" : "Another player's turn")}</p><div className="uno-color-choice">{COLORS.map((color) => <button aria-label={color} className={colorChoice === color ? "active" : ""} key={color} onClick={() => setColorChoice(color)} style={{ background: color }} type="button" />)}</div><div className="uno-hand">{hand.map((card) => <button className={`uno-card uno-card--${card.color ?? "wild"}`} disabled={!myTurn || !top || !canPlay(card, top, state.activeColor)} key={card.id} onClick={() => play(card)} type="button"><strong>{label(card)}</strong></button>)}</div>{state.uno.includes(me) && <strong className="uno-callout">UNO!</strong>}</>}
    {state.phase === "finished" && <h3>{state.winner === me ? (ru ? "Ты победил!" : "You won!") : `${ru ? "Победитель" : "Winner"}: ${state.winner.slice(-6)}`}</h3>}
