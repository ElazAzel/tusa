...
export default function BrainBurst({ partyId, sessionId, onSave, role }: { partyId: string; sessionId?: string | null; onSave: (score: number) => void; role?: "stage" | "controller" }) {
  ...
  const stageHook = useStageGame<GameState>(isHost ? (sessionId ?? null) : null, () => ({ round: 0, phase: "question", question: questions[0].q, options: questions[0].opts, correct: questions[0].correct, timer: 10, scores: {}, locked: {} }));
  const controllerHook = useControllerGame<GameState>(!isHost ? (sessionId ?? null) : null, { round: 0, phase: "question", question: questions[0].q, options: questions[0].opts, correct: questions[0].correct, timer: 10, scores: {}, locked: {} });
  ...
  const next = useCallback(() => {
    ...
    if (nextRound >= questions.length) { complete?.(); const topScore = Object.values(state.scores).reduce((a, b) => Math.max(a, b), 0); onSave(topScore); return; }
  ...
  return <div className="party-game-board game-board-enter">
    ...
    {state.phase === "question" && <>
      <p style={{ fontSize: 48, fontWeight: 700, color: state.timer <= 3 ? "var(--red)" : "var(--lime)" }}>{state.timer}s</p>
      <div className="quiz-options">{state.options.map((opt, i) => <button key={i} className={chosen === i ? "selected" : ""} disabled={chosen !== null} onClick={() => answer(i)} type="button">{opt}</button>)}</div>
      {chosen !== null && <p className="controller-answered">{t("answered")}</p>}
    </>}
    {state.phase === "results" && <div>
      <p style={{ color: "var(--lime)", fontWeight: 700 }}>{t("correct")}: {state.options[state.correct]}</p>
      {sorted.length > 0 && <div style={{ marginTop: 12 }}>{sorted.map(([uid, score], i) => <p key={uid}>{i + 1}. {uid.slice(0, 8)} — {score} {t("pts")}</p>)}</div>}
      {isHost && <div className="game-primary-actions"><button className="demo-action demo-action--lime" onClick={next} type="button">{state.round >= questions.length - 1 ? t("finish") : t("next")}</button></div>}
    </div>}
  </div>;
}
