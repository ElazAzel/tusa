...
function cors(res: NextResponse) { res.headers.set("Access-Control-Allow-Origin", "*"); res.headers.set("Access-Control-Allow-Methods", "GET,POST,DELETE,OPTIONS"); res.headers.set("Access-Control-Allow-Headers", "*"); return res; }
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return cors(NextResponse.json({ highlights }));
...
export async function POST(request: Request) {
  ...
  if (!body.partyId || !body.type) return cors(NextResponse.json({ error: "Укажите partyId и type." }, { status: 400 }));
  try {
    ...
    if (!highlight) return cors(NextResponse.json({ error: "Не удалось сохранить." }, { status: 500 }));
    publish(`party:${body.partyId}`, { type: "highlight:created", highlight });
    return cors(NextResponse.json({ highlight }, { status: 201 }));
...
export async function DELETE(request: Request) {
  ...
  if (!id) return cors(NextResponse.json({ error: "Укажите id." }, { status: 400 }));
  try {
    const ok = await deleteHighlight(id, userId);
    if (!ok) return cors(NextResponse.json({ error: "Не найдено." }, { status: 404 }));
    publish(`party:highlight:${id}`, { type: "highlight:deleted", id });
    return cors(NextResponse.json({ deleted: true }));
  } catch { return cors(NextResponse.json({ error: "Ошибка удаления." }, { status: 500 })); }
}
