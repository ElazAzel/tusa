...
export async function POST(request: Request) {
  ...
  const rl = await distributedRateLimit(`api:promos:redeem:${userId}:${getClientIp(request.headers)}`, 5, 60000);
...
export async function GET(request: Request) {
  ...
  const rl = await distributedRateLimit(`api:promos:redeem:${userId}:${getClientIp(request.headers)}`, 60, 60000);
  if (!rl.allowed) return NextResponse.json({ error: "Слишком много запросов." }, { status: 429 });
