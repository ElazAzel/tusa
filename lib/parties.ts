...
export function ensurePartyV2() {
  ...
  await sql`CREATE UNIQUE INDEX IF NOT EXISTS game_scores_mutation_idx ON game_scores (session_id, client_mutation_id) WHERE client_mutation_id IS NOT NULL`;
  })().catch((error) => {
...
export function ensurePartySchema() {
  ...
  if (process.env.TUSA_STRICT_SCHEMA === "true") {
    ...
    if (!version || Number(version.version) < 9) throw new Error("Database schema is outdated. Run npm run db:migrate before serving traffic.");
    return;
  }
