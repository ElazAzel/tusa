...
function shuffled(players:string[], seed:number) { const order=[...players]; let value=(Math.abs(Math.trunc(seed)) % 2147483646) + 1; for (let index=order.length-1; index>0; index-=1) { value=(value*16807) % 2147483647; const swap=value % (index+1); [order[index],order[swap]]=[order[swap],order[index]]; } return order; }
function roleMap(players:string[], seed:number) { return Object.fromEntries(shuffled(players, seed).map((id,index) => [id, index === 0 ? "mafia" : index === 1 ? "doctor" : index === 2 ? "seer" : "villager"] as const)); }
...
export function createNightCouncil(id:GameId) { return defineGame<State>({
  ...
  reducer(state,action,payload,ctx) {
    if(action==="start") { if(ctx.actorId!==ctx.creatorId) return {state,changed:false,error:"Only the stage can start."}; if(state.phase!=="lobby"&&state.phase!=="finished") return {state,changed:false,error:"The game is already running."}; if(ctx.participants.length<5) return {state,changed:false,error:"At least five players are required."}; const roles=roleMap(ctx.participants, ctx.now); return {changed:true,state:{...state,phase:"night",roles,alive:[...ctx.participants],players:ctx.participants,actions:{},votes:{},eliminated:"",winner:""}}; }
  ...
  sanitizeForViewer(state,viewerId) { const roles=state.roles??{}; const visible={...state,roles:state.phase==="reveal"||state.phase==="finished"?roles:roles[viewerId]?{[viewerId]:roles[viewerId]}:{}}; return visible; },
