...
const migrations = migrationTable[0]?.name
  ...
  : [];
const tables = await sql`SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name IN ('admin_members', 'admin_audit_log', 'safety_reports', 'moderation_actions', 'safety_blocks', 'password_reset_tokens', 'email_verification_tokens', 'platform_error_events', 'platform_schema_version') ORDER BY table_name`;
