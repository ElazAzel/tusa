...
- Unit-тесты проверяют только наличие названий игр в коде: 2 теста.
- Mobile E2E охватывает только 4 публичные страницы и три ширины.
- Локальный E2E не запускается без Clerk env.
- `tusa.game` сейчас не резолвится; работает `tusagame.vercel.app`.
- `llms.txt` отсутствует.
- Sitemap содержит только 10 URL и подставляет текущее время как `lastmod`.
- RAG является локальным TF/IDF-индексом исходного кода, а не пользовательской knowledge-системой.
- SSE и rate limit хранят состояние внутри одной Node.js-инстанции.
- Клиент может передавать итоговый score.
- Схема БД создаётся во время запросов через `ensurePartySchema()`.
- i18n хранится в одном файле на 2232 строки и использует cookie вместо локализованных URL.
- `globals.css` содержит пользовательские незакоммиченные изменения — их необходимо сохранить перед рефакторингом.
- GitHub Actions отсутствует.

### Главные продуктовые метрики

- North Star: `Weekly Successful Party Sessions`.
- Успешная туса: минимум 3 участника, минимум одна завершённая игра, минимум два раунда, длительность более 10 минут, отсутствие критического рассинхрона.
- Activation: доля пользователей, дошедших от инвайта до первого игрового действия.
- Time to First Laugh: медиана менее 90 секунд от открытия ссылки.
- Invite conversion, completed rounds, rematch rate, repeat group rate.
- Lost action rate, reconnect success, session recovery, conflict rate.
- D1/D7/D30 retention хостов и повторные тусы одной компании.

## 2. Архитектура и функциональные изменения

### Этап 0 — единый источник истины и безопасный baseline

1. Зафиксировать текущие незакоммиченные изменения и снять визуальные скриншоты ключевых страниц.
2. Создать единый `GameManifest`, из которого генерируются:
   ...
   - возрастной рейтинг;
   - тестовая матрица;
   - документация и sitemap.

3. Зафиксировать фактическое число режимов: 32. Разделить их на категории `full_game`, `quick_tool`, `experimental`, но конечный Definition of Done распространить на все 32.
4. Удалить противоречивые заявления «8 игр», «28 игр» и «32 игры» из разных частей UI.
5. До сертификации показывать режиму честный статус, не выдавая локальный счётчик за multiplayer-игру.
6. Обновлять AGENTS.md, README, RAG-индекс и публичный каталог автоматически из manifest.
7. Создать реестр требований с колонками: функциональность, безопасность, mobile, i18n, accessibility, analytics, tests, владелец и статус.

### Этап 1 — production-инфраструктура

#### База данных

- Перейти с runtime DDL на Drizzle migrations для Neon.
- Создать версионируемые миграции, миграционный журнал, staging DB, backup и rollback-процедуру.
- Удалить вызовы `ensurePartySchema()` из каждого запроса после миграции существующей production-базы.
- Добавить индексы для party members, active sessions, pending actions, chat pagination, analytics, promo redemption и RAG vectors.
- Ввести retention-политики для действий игр, чатов, голоса, фотографий и аналитики.
- Все денежные, KOINS и промокодные операции выполнять транзакционно.
